...
public class UsuarioDAO {
	...
	public boolean update(Usuario u) throws SQLException {
		...
		boolean resul = false;
		String sql = "{call usuario_update(?,?,?)}"; // faltaria un interrogante en caso de hacer funcionar la id de vuelta,  (parametro de salida)
		try (Connection conn = ConnectionManager.getConnection(); 
				CallableStatement cs = conn.prepareCall(sql);) {
	...
	public boolean delete( long id ) throws SQLException {
		...
		boolean resul = false;
		String sql = "{call usuario_delete(?)}";
		try (Connection conn = ConnectionManager.getConnection(); 
				CallableStatement cs = conn.prepareCall(sql);) {
			//parametro entrada
			cs.setLong(1, id);
